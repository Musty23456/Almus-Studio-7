...
fun PitchCurveEditor(result: PitchCurveAnalyzer.Result, modifier: Modifier = Modifier) {
    ...
    var manual by remember { mutableStateMapOf<Int, Int>() }
    ...
    Column(modifier) {
        ...
        Canvas(
            ...
            for (m in 36..84 step 12) {
                ...
                drawLine(MaterialTheme.colorScheme.outlineVariant, Offset(0f,y), Offset(w,y), 1f)
            }
            ...
            drawPath(original, MaterialTheme.colorScheme.secondary, style=Stroke(3f, cap=StrokeCap.Round))
            points.forEachIndexed { i,p -> if(p.voiced){ val target=manual[i]?.toFloat() ?: PitchCurveAnalyzer.nearestMidi(p.midi).toFloat(); val q=xy(i,target); drawCircle(MaterialTheme.colorScheme.primary, if(i==selected)7f else 3.5f,q) } }
        }
